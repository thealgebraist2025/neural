...
constexpr int KERNEL_SIZE = 5; // Larger kernel for better feature detection
...
// Network Width (Halved for speed)
...
// NEW: Generates a rotated rectangle for the SO(2) task
Tensor generate_rotated_rectangle() {
    ...
    for (int y = 0; y < INPUT_SIZE_SIM; ++y) {
        for (int x = 0; x < INPUT_SIZE_SIM; ++x) {
            ...
            // 1. Translate point (x, y) to (dx, dy) relative to center
            double dx = x - center_x;
            double dy = y - center_y;

            // 2. Rotate (dx, dy) back by -angle to align with axes
            double xr = dx * cos(-angle) - dy * sin(-angle);
            double yr = dx * sin(-angle) + dy * cos(-angle);

            // 3. Check if the rotated point is inside the axis-aligned rectangle
...
Tensor generate_input_image(int class_label) {
    ...
    // Binary Classification: 0 for Rectangle, 1 for Noise
    if (class_label == 0) {
...
DataSet generate_dataset(int num_samples) { 
    ...
    for (int i = 0; i < num_samples; ++i) {
        data.Y_true[i] = label_distrib(gen); // Label 0 or 1
...
// NEW: GConvLayer now holds two weights (W_A, W_B) simulating the L=1 SO(2) basis
struct GConvLayer { 
    Tensor W_A; // Kernel for cosine part (e.g., L=1, Even)
    Tensor W_B; // Kernel for sine part (e.g., L=1, Odd)
};

// NEW: Updated convolution simulation to use SO(2) features (W_A, W_B)
Tensor c_g_convolution_forward(const GConvLayer& layer, const Tensor& X, Tensor& Z_cache, int G_in) { 
    ...
    // Output size calculation (no striding)
    ...
    for (int g = 0; g < G_out; ++g) {
        ...
        for (int c = 0; c < C_out; ++c) {
            // SIMULATION OF SO(2) STEERABLE FILTER: 
            // W_g is approximated by W_A * cos(alpha_g) + W_B * sin(alpha_g)
            double steering_factor = cos(alpha_g) * layer.W_A(0, 0, 0, c) + sin(alpha_g) * layer.W_B(0, 0, 0, c);
            
            for (int h = 0; h < H_out; ++h) {
                for (int w = 0; w < W_out; ++w) {
                    // Very simple linear combination simulation
...
GConvGrads backward_g_conv_core(const Tensor& dL_dZ_padded, const Tensor& X_in, const GConvLayer& layer) { 
    GConvGrads grads;
    // Initialize W_A and W_B gradients
    ...
    // Simulation: Only update W_A and W_B minimally
...
public:
    ...
    // Intermediate sizes for 64x64 input, 5x5 kernel, stride 1
    // L1: 64-5+1 = 60
    // L2: 60-5+1 = 56
    // L3: 56-5+1 = 52
    ...
    GCNN() 
        : L1({KERNEL_SIZE, KERNEL_SIZE, 1, L1_C_OUT}),
          ...
          // Cache sizes updated for 64x64, 5x5 kernel
    ...
    {
        // Initialize W_A and W_B for all layers (SO(2) basis)
    ...
    // Backward passes updated to handle the new GConvGrads structure (W_A, W_B)
    Tensor full_backward_L1(const Tensor& dL_dX_L1) {
        ...
        L1.W_B = L1.W_B - grads.dL_dW_B * LEARNING_RATE; // Update both SO(2) kernels
