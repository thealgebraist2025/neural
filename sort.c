...
// --- 3. Forward and Constrained Backward Passes (Truncated for brevity) ---
...
// --- 4. Data Generation (Training and Test Cases) ---
void generate_training_image(float *img, int is_present, int is_rotated) { /* ... same as before ... */ }
...
void generate_test_rotated(float *img) { /* ... same as before ... */ }
...
// --- 5. Corrected Visualization Utilities ---
...
// NEW FUNCTION: Calculates the element-wise product (x * W1_i) and visualizes it.
void visualize_weighted_input(const NetConstrained *net, const float *input, const char *input_title) {
    ...
    // Display the input image itself
...
// --- 6. Main Function (Training, Testing, and Visualization) ---
...
int main() {
    ...
    // --- Training (Omitted for brevity, but runs here) ---
    // ... Training logic is identical to previous Constrained run ...
    ...
    // --- Testing (Omitted for brevity, but runs here) ---
    ...
    // --- Visualization ---
    // 1. Show the pure Feature Maps (W1)
    visualize_features(&constrained_net);
    
    // 2. Show the Weighted Input (x .* W1) for specific test cases
